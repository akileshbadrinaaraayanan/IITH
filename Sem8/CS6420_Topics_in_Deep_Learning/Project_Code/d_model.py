...
class DiscriminatorModel:
    ...
    def define_graph(self):
        ...
        with tf.name_scope('discriminator'):
 	    ...
 	    self.scale_nets2 = []
	    for scale_num in xrange(self.num_scale_nets):
                with tf.name_scope('scale_net_2_' + str(scale_num)):
                    scale_factor = 1. / 2 ** ((self.num_scale_nets - 1) - scale_num)
                    self.scale_nets2.append(DScaleModel(scale_num,
                                                       int(self.height * scale_factor),
                                                       int(self.width * scale_factor),
                                                       self.scale_conv_layer_fms[scale_num],
                                                       self.scale_kernel_sizes[scale_num],
	    ...
	    # A list of the prediction tensors for each scale network
            self.scale_preds2 = []
            for scale_num in xrange(self.num_scale_nets):
                self.scale_preds2.append(self.scale_nets2[scale_num].preds)
	
	  # Data 
	    self.labels2 = tf.placeholder(tf.float32, shape=[None, 1], name='labels2')
	    ...
	    ##
            ...
            with tf.name_scope('training'):
                ...
                self.global_loss = (adv_loss(self.scale_preds, self.labels)+adv_loss(self.scale_preds2, self.labels2))/(2.0)
    ...
    def build_feed_dict(self, input_frames, gt_output_frames, blurs, generator):
        """
        ...
        for scale_num in xrange(self.num_scale_nets):
            ...
            scale_net2 = self.scale_nets2[scale_num]
	    ...
	    #resize blur output frames
	    scale_blurs = np.empty([batch_size, scale_net.height, scale_net.width, 3])
	    ...
	    scaled_input_frames2 = np.concatenate([scale_blurs,
                                                  scaled_gt_output_frames])

            # convert to np array and add to feed_dict
            feed_dict[scale_net.input_frames] = scaled_input_frames
	    feed_dict[scale_net2.input_frames] = scaled_input_frames2
	...
	feed_dict[self.labels2] = np.concatenate([np.zeros([batch_size, 1]),
                                                 np.ones([batch_size, 1])])

        return feed_dict

    def train_step(self, batch, blurs, generator):
        """
        ...
        feed_dict = self.build_feed_dict(input_frames, gt_output_frames, blurs, generator)
	    ...
	    f = open("stats_discriminator.txt", "a")
	    f.write(str(global_step) + "\t" + str(global_loss) + "\n")
