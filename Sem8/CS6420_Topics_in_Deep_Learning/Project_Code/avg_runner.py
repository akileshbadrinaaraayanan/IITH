...
from utils import get_train_batch, get_test_batch, check_nan_inf, get_recursive_train_batch, get_recursive_test_batch, get_original_test_batch
...
class AVGRunner:
    ...
    def train(self):
	    ...
	    # batch = get_recursive_train_batch()
            ...
            self.global_step = self.g_model.train_step(
                batch, blurs, discriminator=(self.d_model if c.ADVERSARIAL else None))
    ...
    def test(self):
        ...
        """
        batch = get_recursive_test_batch(c.BATCH_SIZE, num_rec_out=self.num_test_rec)
    ...
    def test_original(self):
        ...
        for i in range(0, test_dir_len, c.BATCH_SIZE):
            if i+c.BATCH_SIZE<test_dir_len:
                ...
                batch = get_original_test_batch(c.TEST_ORIGINAL_DIR, c.BATCH_SIZE, i, i+c.BATCH_SIZE)
                psnr, sharpdiff = self.g_model.test_batch(
                    batch, self.global_step, num_rec_out=self.num_test_rec, save_imgs=False, test_only=True, model_no=model_no, start=i)
        ...
        f.write(str(self.model_name)+"\t"+str(psnr_mean)+"\t"+str(sharpdiff_mean))
        f.close()
