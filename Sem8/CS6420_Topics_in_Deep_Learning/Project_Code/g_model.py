...
from loss_functions import combined_loss2
...
class GeneratorModel:
    ...
    def define_graph(self):
	    ...
	    self.d_scale_preds2 = [] # the predictions from the discriminator model   
            ...
            for scale_num in xrange(self.num_scale_nets):
			    ...
			    self.d_scale_preds2.append(tf.placeholder(tf.float32, [None, 1]))
            ...
            with tf.name_scope('train'):
                ...
                self.global_loss = combined_loss2(self.scale_preds_train,
                                                 self.scale_gts_train,
                                                 self.d_scale_preds, self.d_scale_preds2)
    ...
    def train_step(self, batch, blurs, discriminator=None):
        """
	    ...
	    d_feed_dict2 = {}
            for scale_num, gen_frames in enumerate(scale_preds):
          	...
          	scale_factor = 1. / 2 ** ((self.num_scale_nets - 1) - scale_num)
                ...
                scale_blurs = np.empty([len(blurs), scale_height, scale_width, 3])
                for i, img in enumerate(blurs):
                    # for skimage.transform.resize, images need to be in range [0, 1], so normalize to
                    # [0, 1] before resize and back to [-1, 1] after
                    sknorm_img = (img / 2) + 0.5
                    sknorm_img[sknorm_img > 1.0] = 1.0
                    sknorm_img[sknorm_img < 0.0] = 0.0
                    resized_frame = resize(sknorm_img, [scale_height, scale_width, 3])
                    scale_blurs[i] = (resized_frame - 0.5) * 2
	    	
		d_feed_dict2[discriminator.scale_nets2[scale_num].input_frames] = scale_blurs 
	  
	    d_scale_preds = self.sess.run(discriminator.scale_preds, feed_dict=d_feed_dict)
	    d_scale_preds2 = self.sess.run(discriminator.scale_preds2, feed_dict=d_feed_dict2)

            # Add discriminator predictions to the
            for i, preds in enumerate(d_scale_preds):
                feed_dict[self.d_scale_preds[i]] = preds

	    for i, preds in enumerate(d_scale_preds2):
                feed_dict[self.d_scale_preds2[i]] = preds
	    ...
	    f = open("stats_generator.txt", "a");
	    f.write(str(global_step) + "\t" + str(global_loss) + "\t" + str(global_psnr_error) + "\t" + str(global_sharpdiff_error) + "\n")
    ...
    def test_batch(self, batch, global_step, num_rec_out=1, save_imgs=True, test_only=False, model_no=1, start=0):
        """
        Runs a training step using the global loss on each of the scale networks.
        @param batch: An array of shape
	    ...
	    f = open("stats_test_generator.txt", "a")
	    f.write(str(global_step) + "\t" + str(psnr) + "\t" + str(sharpdiff) +"\n")
		...
		if test_only==True:
		    pred_dir = c.get_dir(os.path.join(
                    c.IMG_SAVE_DIR, 'Tests_original/' + str(model_no), str(start+pred_num)))
