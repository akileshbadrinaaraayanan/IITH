...
if args.model is None:
    for i in xrange(args.epoch):
        ...
        for batch_id, xy in data_xy.items():
            ...
            if (i+1)%200 == 0:
                f = open('data/outputsent_'+str(i+1)+'_'+args.path.split('/')[-1], 'w')
...
#            lr*=0.1
    ...
    save_model("./model/hedsent_"+save_time+".model", model)

else:
    load_model(args.model, model)
f = open('data/outputsent_'+args.path.split('/')[-1], 'w')
for batch_id, xy in data_xy.items():
    X = xy[0]
    mask = xy[2]
    local_batch_size = xy[3]
