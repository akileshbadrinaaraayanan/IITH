...
def write_to_file(f_path):
    f = open(curr_path + "/" + f_path, "r")
    out = open(curr_path + "/" + f_path[:-4] + "_hrishi.txt", "w")
    for line in f:
        line = line.strip('\n').lower()
        words = line.split()
        words.append("<eoss>") # end symbol
        if len(words) < 3 or len(words)>20 or line == "===":
            continue
        for w in words[:-1]:
            out.write(w+" ")
        out.write("\n")
...
def word_sequence(f_path, batch_size = 1):
    ...
    lines = []
    tf = {}
#    if os.path.exists(f_path[:-3]+'npz'):
#        data = np.load(f_path[:-3]+'npz')
#        return data['seqs'].tolist(), dict(data['i2w']), dict(data['w2i']), data['data_xy']
    f = open(curr_path + "/" + f_path, "r")
    for line in f:
        line = line.strip('\n').lower()
        words = line.split()
    ...
    #np.savez(f_path[:-3]+'npz', seqs=seqs, i2w=i2w, w2i=w2i, data_xy=data_xy)
