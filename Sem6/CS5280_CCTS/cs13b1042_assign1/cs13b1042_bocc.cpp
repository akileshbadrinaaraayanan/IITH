...
void BOCC::read(int id, int x, int &l)
	{
...
void BOCC::write(int id, int x, int l)
	{
		//pthread_mutex_unlock(&lock);
		buffer[id][x] = l;
		//pthread_mutex_unlock(&lock);
			...
			if(txEndTime[i] > txStartTime[id])
				{
					for(int j=0; j<m; j++)
					{
						if(readSet[id][j] == writeSet[i][j] && writeSet[i][j] == 1)
							toAbort = 1;
					}
				}
		}
