...
void MVTO::read(int id, int x, int &l)
	{
	...
	//I check for the largest time stamp less than that of timestamp of id
	//Update readset accordingly	
		int req = 0, i;
		...
		for(int j=i+1; j<n; j++)
		{
			if(txStartTime[j] > txStartTime[req] && txStartTime[j] < txStartTime[id])
			{
				req = j;
			}
		}
...
void MVTO::write(int id, int x, int l)
	{
		pthread_mutex_lock(&lock);
		buffer[id][x] = l;
		// pthread_mutex_unlock(&lock);
...
int MVTO::tryC(int id, int type, int rndId)
{
	int toAbort = 0;
	//Notice the lock
	pthread_mutex_lock(&lock);
	...
	if(type == 1)
	{
		...
		for(int j=0; j< id; j++)
		{
			//cout << "err " << readSet[j][rndId];
				if(readSet[j][rndId] != 0)  
				{
					if(readSet[j][rndId] >= n)
						{
							cout << "fail" <<endl;
							return 0;
						}
					if((txStartTime[readSet[j][rndId]] < txStartTime[id]) && (txStartTime[id] < txStartTime[j]))
						toAbort = 1;
				}
		}	
	}
